...
class _EditVehicleScreenState extends State<EditVehicleScreen> {
  ...
  void initState() {
    ...
    _initialOdometerController = TextEditingController(
      text: widget.vehicle.initialOdometerKm?.toString() ?? '',
    );
    _purchasePriceController = TextEditingController(
      text: widget.vehicle.purchasePrice?.toString() ?? '',
    );
    _purchaseDate = widget.vehicle.purchaseDate;
    _lastInspectionDate = widget.vehicle.lastInspectionDate;
  ...
  Future<void> _loadVehicleFuels() async {
    try {
      final fuels = await _vehicleService.getVehicleFuels(widget.vehicle.id);
      if (mounted) {
        setState(() {
          _selectedFuels.clear();
          _selectedFuels.addAll(fuels);
          _primaryFuel = fuels
              .firstWhere(
                (f) => f.isPrimary,
                orElse: () => VehicleFuelConfig(fuel: '', isPrimary: false),
              )
              .fuel;
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      final vehicleUpdate = VehicleUpdate(
      ...
      await _vehicleService.updateVehicle(widget.vehicle.id, vehicleUpdate);

      // Update fuel configuration if any fuels selected
      if (_selectedFuels.isNotEmpty && mounted) {
        await _vehicleService.replaceVehicleFuels(
          widget.vehicle.id,
          _selectedFuels,
        );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to update vehicle: ${e.toString()}'),
            backgroundColor: AppColors.error,
          ),
        );
      }
                    ...
                    children: [
                      ...
                      TextFormField(
                        ...
                        validator: (value) {
                          if (value != null && value.length > 120) {
                            return 'Policy number must be less than 120 characters';
                          }
                          return null;
                        },
                      ),
                      ...
                      TextFormField(
                        ...
                        decoration: const InputDecoration(
                          ...
                          hintText: 'Odometer reading when purchased',
                        ),
